...
class AdminController extends Controller
{
    ...
    public function sendPasswordResetLink(Request $request){
        $request->validate([
            ...
            'email.exists'=>'This :attribute does not exist in system.'
        ]);
        ...
        $mailConfig = array(
            'mail_from_email'=>env('EMAIL_FROM_ADDRESS','info@ecommerce.com'),
            'mail_from_name'=>env('EMAIL_FROM_NAME','laravelcom'),
    ...
    public function changeProfilePicture(Request $request)
    {
        ...
        dd($file_path);
        $filenamne = 'ADMIN_IMG_'.rand(2,1000).$admin->id.time().uniqid().'.jpg';

        $upload = $file->move(public_path($path),$filenamne);

        if($upload){
            if($old_picture != null && File::exists(public_path($file_path))){
                File::delete(public_path($file_path));
            }
            $admin->update(['picture'=>$filenamne]);
            return response()->json(['status'=>1,'msg'=>'Your profile picture has been successfully updated']);
        }else{
            return response()->json(['status'=>0,'msg'=>'Some thing went wrong']);
        }
    }
}
